...
export default function LibraryPage() {
  ...
  const handleReadBook = (book: any, e: React.MouseEvent) => {
    ...
    const url = `http://127.0.0.1:8080/api/document/proxy/epub?url=${encodeURIComponent('https://www.gutenberg.org/ebooks/' + book.gutenberg_id + '.epub.images')}`;
    ...
    <div className="pb-12">
      <div className="relative mb-10 overflow-hidden rounded-3xl bg-black text-white p-8 sm:p-12 shadow-xl">
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          <div className="max-w-2xl">
            <h1 className="text-4xl font-extrabold tracking-tight mb-4">Discover Your Next Book</h1>
            <p className="text-gray-400 text-lg leading-relaxed">
              Explore our curated library of top literature. Instantly generate audio, mindmaps, and summaries with a single click.
            </p>
          </div>
      ...
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-xl font-bold text-gray-900 flex items-center gap-2">
          Available Books <span className="px-2.5 py-0.5 rounded-full bg-gray-100 text-gray-600 text-sm">{filteredBooks.length}</span>
        </h2>
      </div>
          ...
          return (
            ...
            >
              ...
              <div className="p-5 flex-1 flex flex-col">
                ...
                <p className="text-sm text-gray-500 line-clamp-1">{book.author || 'Unknown Author'}</p>
                ...
                <div className="mt-3 flex items-center gap-1.5 text-xs text-gray-400 font-medium bg-gray-50/50 w-fit px-2 py-1 rounded border border-gray-100">
                  <Clock size={12} />
                  Est. process: {calculateEstimatedTime(book.page_count || 200)}
                </div>

                <div className="mt-auto pt-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-1.5">
                      <div className="w-7 h-7 rounded-full bg-blue-50 text-blue-600 flex items-center justify-center cursor-pointer hover:bg-blue-100 transition-colors" title="Read EPUB" onClick={(e) => handleReadBook(book, e)}>
                        <BookOpen size={12} />
                      </div>
                      <div className="w-7 h-7 rounded-full bg-purple-50 text-purple-600 flex items-center justify-center" title="Audio Generation">
                        <Play size={12} fill="currentColor" />
                      </div>
                      <div className="w-7 h-7 rounded-full bg-orange-50 text-orange-600 flex items-center justify-center" title="Interactive Mindmap">
                        <Map size={12} />
                      </div>
                    </div>
                    <button 
